...
export default function MapLegend() {
  ...
  return (
    <Panel position="bottom-right" style={{ padding: 0, margin: "0 0 24px 0" }}>
      {/*
       * align-items: stretch → left column grows to match panel content height.
       * Tab is absolutely positioned at 30% down within that column.
       * pointer-events: none on invisible areas so the canvas stays clickable.
       */}
      {/* drop-shadow on the whole assembly so open+closed render as one shape */}
      <div
        className="flex items-stretch"
        style={{ pointerEvents: "none", filter: "drop-shadow(0 2px 8px rgba(0,0,0,0.12))" }}
      >
        {/* ── Left column: stretches to content height, tab at ~30% ── */}
        ...
        {/* ── Sliding panel — width clips; height follows content ── */}
        <div
          style={{
            width: open ? 290 : 0,
            overflow: "hidden",
