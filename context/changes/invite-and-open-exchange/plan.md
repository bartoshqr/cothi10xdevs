...
S-02 introduces the **second participant** into a data model that is, today, strictly single-owner. The advocate opens an *exchange* on an existing debate (gated on a root Claim existing), sets a round count (1–5, default 3, fixed at initiation), searches a registered user by username, and sends an in-app invite the challenger can accept or decline. Accepting makes the challenger a participant and — via a widened RLS predicate — unlocks read access to the debate graph that is owner-only today.

Covers PRD **FR-007** (root-Claim initiation gate), **FR-008** (round count + challenger-first/advocate-last ordering), **FR-009** (username search + invite), **FR-010** (accept/decline). Roadmap slice **S-02** (`invite-and-open-exchange`).
...
- **FR-007 gate is cheap and already-guaranteed.** `debates.root_node_id IS NOT NULL` is authoritative: creation always sets it (`create_debate_with_root`, `20260528000001:156-204`), the root can't be deleted (FK backstop → 409, `repository.ts:144-159`) or demoted (`repository.ts:108-118`).
- **FR-009 search primitive already ships.** `findUserByUsername` (`src/lib/users.ts:9-20`) — exact, case-insensitive (`.eq` on the normalized lowercased value, deliberately not `.ilike` because usernames may contain `_`). `profiles_select_authenticated using(true)` (`20260525142850_create_profiles.sql:20-26`) lets any authed user resolve it.
...
An advocate viewing their own debate (with a root Claim) can search a username, pick a round count, and send an invite. The invited user sees it on a `/invites` page and can Accept or Decline. On Accept, the challenger can read the debate graph (RLS now permits it); on Decline, they cannot, and the advocate may re-invite (a fresh exchange). A non-participant third user can never read the debate. The FR-007 gate, self-invite block, and one-open-exchange-per-debate rule are all enforced server-side, not just in the UI.

**Verify:** the integration suite in Phase 5 proves the RLS pair-visibility matrix, the gate, and the invite semantics; `npm run build` + `npx astro check` pass; manual click-through of search→invite→accept works in two browser sessions.

### Key Discoveries:

- The hard part is **rewriting the membership predicate consistently** across `debates` + the `nodes`/`relations` subqueries, not adding columns (`20260528000001:98-105,125-132`).
- The FR-007 gate maps to a single predicate `root_node_id IS NOT NULL` (`20260528000001:27`) — enforce server-side (roadmap:148 "thin maps reach challengers" risk).
...
- `findUserByUsername` (`src/lib/users.ts:9-20`) is the entire FR-009 substrate — reuse, do not reimplement.
...
- **No turn-submission machinery** (marking, lock-on-submit, round advance) — that is S-03/S-04. S-02 only *stores* the round count and sets the challenger-first starting marker.
...
A new `exchanges` table carries the second participant (`challenger_id`), the invite lifecycle (`status` pending→accepted/declined), the round count, and a challenger-first turn marker. A SQL helper `is_debate_participant(p_debate_id)` centralizes the "owner OR accepted challenger" check so the widened RLS predicate is written once and reused across `debates`/`nodes`/`relations`. DB constraints (round-count CHECK, self-invite CHECK, partial-unique on one-open-exchange) are backstops; the repository pre-checks app-side to surface clean typed errors (mirroring the `createRelation`/`updateNode` pattern). Endpoints follow `withAuth`; search returns 200-with-null on no match; the gate is enforced server-side and reflected in the UI.

## Critical Implementation Details

- **State sequencing — accept must precede visibility.** The widened RLS predicate keys off `status = 'accepted'`. A `pending` invite must NOT grant graph read access (challenger cannot preview the map before accepting). The `respond` transition flipping `pending→accepted` is therefore the exact moment visibility opens — there is no separate "grant access" step.
...
- **Self-invite is checked in two places by design.** App-side (`advocate_id === challenger_id` → `ValidationError` 422, clean message) AND a DB `CHECK (advocate_id <> challenger_id)` backstop. The app check fires first; the DB check guards against a direct write.
- **`is_debate_participant` must be `security definer` / `set search_path = ''`** and used inside RLS policies so the membership subquery reads the exchange table regardless of the caller's own RLS on `exchanges`. Mirror the grant pattern from `create_debate_with_root` (`20260528000001:202-204`).

## Phase 1: Schema + RLS migration

### Overview

Create the `exchanges` table and its enum, the DB constraints, the `is_debate_participant` helper, and rewrite the owner-only RLS on `debates`/`nodes`/`relations` to a participant predicate. Regenerate types. This is the load-bearing phase — the RLS rewrite is the slice's central risk.
...
- `public.exchanges` columns: `id uuid pk default gen_random_uuid()`, `debate_id uuid not null references public.debates on delete cascade`, `advocate_id uuid not null references auth.users on delete cascade`, `challenger_id uuid not null references auth.users on delete cascade`, `status public.exchange_status not null default 'pending'`, `round_count int not null`, `current_turn public.turn_actor not null default 'challenger'`, `created_at timestamptz not null default now()`, `responded_at timestamptz null`.
...
#### 2. Same migration — `is_debate_participant` helper

**File**: same migration

**Intent**: Centralize the "owner OR accepted challenger" membership test so the RLS predicate is defined once.

**Contract**: `create function public.is_debate_participant(p_debate_id uuid) returns boolean language sql security definer set search_path = '' stable` returning true when the current `auth.uid()` is the debate owner OR an accepted challenger:
```sql
select exists (
  select 1 from public.debates d where d.id = p_debate_id and d.owner_id = auth.uid()
) or exists (
  select 1 from public.exchanges e
  where e.debate_id = p_debate_id and e.challenger_id = auth.uid() and e.status = 'accepted'
);
```
`revoke execute ... from public, anon; grant execute ... to authenticated;` (mirror `20260528000001:202-204`).
...
**Intent**: Replace the owner-only *read* predicate with participant membership so an accepted challenger can read the graph. Write policies stay author-scoped (challenger writes are S-03).

**Contract**: drop & recreate the three `*_select` policies to use the helper:
- `debates_select`: `using (public.is_debate_participant(id))`.
- `nodes_select`: `using (public.is_debate_participant(debate_id))`.
- `relations_select`: `using (public.is_debate_participant(debate_id))`.

Leave `*_insert` / `*_update` / `*_delete` on `debates`/`nodes`/`relations` unchanged (still owner/author-scoped) — challenger write access is out of scope for S-02.
...
Create `src/lib/exchange/` mirroring `src/lib/debate/`: centralized round-count constants, Zod schemas, and a repository with app-side pre-checks. No HTTP yet.
...
**Contract**: `ROUND_COUNT = { min: 1, max: 5, default: 3 } as const;`.
...
- `usernameSearchSchema`: reuse `USERNAME_PATTERN` (`src/lib/username.ts`) — a query param validated as a username string.
...
- `openExchange(supabase, input, advocateId)`: (a) load debate by `input.debateId` (RLS-scoped) → null ⇒ `NotFoundError` (404); (b) `root_node_id == null` ⇒ `ValidationError` (422, FR-007 gate); (c) `input.challengerId === advocateId` ⇒ `ValidationError` (422, self-invite); (d) insert the exchange row (status defaults `pending`); map SQLSTATE `23505` (partial-unique) ⇒ `ConflictError` (409, "An exchange is already open on this debate."). Mirror `createRelation` (`repository.ts:188-223`).
...
**Contract**: round count rejects 0 and 6, accepts 1/3/5; default surfaced from `ROUND_COUNT`; `.strict()` rejects unknown fields; respond requires a boolean.
...
**Intent**: Resolve a username to `{ id, username } | null` for the invite search box; soft "not found" (no error).

**Contract**: `export const GET = withAuth(...)`. Read `context.url.searchParams.get("username")` (template: `src/pages/api/auth/username-available.ts:5-22`, but `withAuth`-wrapped). Validate against `usernameSearchSchema` (invalid → 400). Call `findUserByUsername`. **Always 200**: `{ user: { id, username } | null }`.
...
**Intent**: Let the advocate search a username, choose a round count (1–5, default 3 from `ROUND_COUNT`), and send the invite. Reflect FR-007 gate and existing-exchange state in the UI (server remains authoritative).

**Contract**: a React island (`client:load`/`client:only`) shown only to the debate owner. Username input → debounced `GET /api/users/search` → show resolved user or soft "no user found". Round-count selector defaulting to `ROUND_COUNT.default`. Submit → `POST /api/exchanges`. Disable/hide when the debate has no root Claim (gate); when an exchange already exists, show its status instead of the form. Surface 409/422 via the existing `apiError` helper (`src/components/debate/apiError.ts`). The page already loads the graph (`[id].astro:13`) and root state — pass `hasRoot` / existing-exchange status as props.
...
- **Gate (FR-007):** open against a debate whose `root_node_id` is null (insert via service client) ⇒ `ValidationError`. A debate with a root ⇒ succeeds.
- **RLS pair-visibility (Risk #1):** advocate opens exchange inviting userB. *Before accept:* userB's as-user client SELECT on the debate/nodes/relations ⇒ empty/denied. *After accept* (userB responds): userB CAN read all three. A third non-participant user ⇒ denied throughout.
- **Self-invite:** `advocateId === challengerId` ⇒ `ValidationError` (422).
...
- **Respond transitions:** accept flips `status='accepted'` + sets `responded_at`; decline flips `status='declined'`; responding to a non-pending exchange ⇒ `NotFoundError` (404).
...
- Exchange schemas: round-count bounds (reject 0/6, accept 1/3/5), `.strict()` rejects unknown fields, respond requires boolean.

### Integration Tests (the slice's real signal):

- FR-007 gate rejection server-side.
- RLS pair-visibility: challenger reads only after accept; non-participant always denied (two-user + as-user fixture).
- Self-invite (422), duplicate-open (409), re-invite-after-decline (success).
...
- The widened RLS predicate adds an `exchanges` subquery to every graph read. `is_debate_participant` short-circuits on the owner branch (the common case) before touching `exchanges`; the `(challenger_id)` and `(debate_id)` indexes keep the challenger branch cheap. Graph reads are per-debate and small (MVP scale).

## Migration Notes

- Single forward migration; no existing data to migrate (no exchanges exist). The RLS `*_select` policies are dropped and recreated — existing owner access is preserved by the helper's owner branch.
...
- [ ] 1.5 RLS pair-visibility behaves (accepted challenger reads; non-participant denied)

  > **Agent-automatable**: Yes — two anon-key sessions via the password grant + SQL `set local role`.

  ```sql
  -- As an accepted challenger (set local role to their uid), expect 1 row:
  ...
  -- Expected: 1 when status='accepted', 0 when 'pending'/'declined'.
...
- [ ] 3.4 Endpoints return expected status codes (happy + edge)
  ...
  # Search (200 with user or null):
  curl -s "http://localhost:4321/api/users/search?username=<name>" -H "Authorization: Bearer $TOKEN"
  ...
  # Self-invite (expect 422); duplicate open (expect 409); gateless debate (expect 422).
  ```
...
- [ ] 4.4 Two-session click-through: search → invite → accept → debate readable; decline → re-invite

  > **Agent-automatable**: No — requires two browser sessions and visual confirmation of the invite UI.

- [ ] 4.5 Invite affordance hidden/disabled when no root Claim exists
