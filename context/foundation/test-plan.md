...
>
> Last updated: 2026-06-20 (refresh: gates lifted — S-01..S-06 + north star S-04 all shipped; added Risk #7 critical-path e2e; Phase 5 promoted + Playwright scaffolded)
...
| 4   | A party **mutates a locked statement** (after submitting their turn, or outside their active turn) or acts out of round order — turn / round integrity corrupts the exchange.                                                                                                                                                                                                               | High   | High       | interview Q1 (top worry); PRD FR-026, FR-017 — _mechanic shipped in roadmap S-03/S-05 (now `done`)_                                                                                                                                                    |
| 5   | An edit or delete **corrupts the graph**: orphaned nodes left unflagged, dangling relations, or marks left pointing at deleted / edited statements (silent data loss).                                                                                                                                                                                                                      | High   | High       | PRD US-04, FR-026; roadmap S-05 ("heaviest, riskiest slice"); interview Q3 — _mechanic shipped in roadmap S-05 (now `done`)_                                                                                                                           |
...
| 7   | The **full exchange lifecycle breaks at a boundary crossing** — advocate builds a graph → invites → challenger marks/submits → advocate marks/submits → divergence summary renders — even though every layer passes in isolation. The break lives where layers _compose_: auth↔routing, store↔API, API↔DB, SSR↔island handoff, round-to-round state, and survival across real page reloads. | High   | Medium     | roadmap North star / Primary Success Criterion ("build → invite → challenger marks/adds → advocate responds → summary"); user-stated top concern (2026-06-20); 16 unit + 11 integration files exist, **zero true e2e** — the composed path is untested |
...
Risks #4 and #5 cover the turn-locking (roadmap S-03) and edit/delete
mark-invalidation (roadmap S-05) state machines. As of 2026-06-20 **both
slices have shipped** (S-01..S-06 and the north star S-04 are all `done` in
`roadmap.md`), so the former gate on Phase 4 is lifted — the code is on disk
and `/10x-research` has something to ground. Those phases remain `not started`
as _test_ phases (shipping a feature is not the same as opening a test phase),
just no longer blocked. Risk #7 is the new top gap: every layer is covered in
isolation but the **composed** lifecycle has zero e2e — see §3 Phase 5, now
promoted ahead of Phases 2–4.
...
| #7   | A **single browser run** drives the whole flow: signed in as advocate, build a graph → invite a challenger → (as challenger) mark every advocate statement + submit → (as advocate) mark + submit → the **divergence summary renders with the correct common-ground / divergence classification**, and the state **survives a real `page.reload()`** mid-flow (not just in-memory store). | "All unit/integration tests are green, so the chain works" — per-layer correctness does not prove the layers compose through real navigation, SSR↔island hydration, and multi-user sessions. | current route/component names (grounded — see `context/changes/test-plan-refresh-2026-06-20/research.md`): create `/debates/new`, canvas `/debates/{id}` (`MapEditor`), `InviteChallenger`, `StatementNode` mark bar, `TurnBar`, `DivergenceSummary`; cookie-based `@supabase/ssr` auth → Playwright `storageState` feasible; two users via the existing `globalSetup.ts` provisioning. | **e2e (Playwright)** — the one risk in this project that genuinely needs a browser | mocking Supabase auth/DB (mocks away the exact boundary the risk is about); asserting only "page didn't error" instead of the summary's actual `commonGround` / `openDivergences` (factual vs values) classification; driving marks/submit purely via API (re-tests the integration layer, dodges the SSR↔island boundary). |
...
| 4   | Turn / round + edit-delete integrity | Lock turn-locking, mark-invalidation, and orphaning (slices S-03/S-05 now shipped)                                                                                                                          | #4, #5        | integration        | not started | —                                          |
| 5   | Critical-path e2e                    | One Playwright spec drives the full advocate↔challenger lifecycle — debate creation → invite → challenger marks/submits → advocate marks/submits → divergence summary renders, surviving a real page reload | #7            | e2e (Playwright)   | not started | —                                          |

**Status vocabulary** (fixed — parser literals): `not started` →
`change opened` → `researched` → `planned` → `implementing` → `complete`.

**Execution order (updated 2026-06-20): 1 → 5 → 2 → 3 → 4.** Phase numbers
are stable identities (cross-referenced throughout this doc); they are _not_
the run order. Phase 1 is complete. **Phase 5 (critical-path e2e) runs next** —
it is the highest-value gap (the composed lifecycle has zero e2e coverage) and
the slice the team explicitly prioritised this session. Phases 2–4 follow and
are all actionable now: the former gates are **lifted**. Phase 4's risks (#4,
#5) cover the turn / mark-invalidation machine from roadmap slices S-03
(`challenger-first-turn`) and S-05 (`multiround-edit-invalidation`); both are
now `done`, as are S-02 (Phase 2 RLS surface) and S-04 (Phase 5 summary), so
`/10x-research` has real code to ground in every phase.
...
where that fixture already lives. Also note: Phase 1 grew two small _features_
...
| e2e                  | Playwright (`@playwright/test`) | ^1.61.0 | **Scaffolded 2026-06-20** (commit `add playwright`): `playwright.config.js` (testDir `./tests/e2e`, `webServer: npm run dev` @ `http://localhost:4321`, chromium + firefox), throwaway `tests/e2e/example.spec.js` to replace. Reserved for the one critical flow (Risk #7); see Phase 5. |
...
- Runtime/browser: Playwright now scaffolded locally (`playwright.config.js`, `@playwright/test` ^1.61.0); a Playwright MCP (`mcp__playwright__*`) is exposed this session for interactive driving/debugging. The Phase 5 e2e suite itself runs via the local config (`npx playwright test`), not the MCP; checked: 2026-06-20
...
| e2e on the critical flow                       | CI on PR                            | CI                      | required after §3 Phase 5 (S-04 shipped — no longer blocked) | a broken build → invite → mark → summary path (Risk #7)                                                             |
...
- TBD — to be filled when §3 Phase 5 lands. No longer gated: S-04 (and the
  whole S-01..S-06 critical path) has shipped. Grounding for the first spec —
  route/component map, accessible locators, two-user `storageState` approach,
  and the reload-survival assertion — lives in
  `context/changes/test-plan-refresh-2026-06-20/research.md`.
...
- Strategy (§1–§5) last reviewed: 2026-06-20 (Risk #7 added; phase gates lifted; Phase 5 promoted)
- Stack versions last verified: 2026-06-20 (Playwright `@playwright/test` ^1.61.0 scaffolded)
- AI-native tool references last verified: 2026-06-20 (Playwright MCP now exposed)
