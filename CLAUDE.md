...
See @README.md for stack overview, all npm scripts, env setup, Supabase local dev, route protection, deployment, and CI.

## Hard rules

- Never use `set:html` in Astro components (`astro/no-set-html-directive: error`).
...
See @eslint.config.js and @.prettierrc.json for the full config. Key rules to keep in mind:

- Path alias: `@/*` → `./src/*`
- Husky pre-commit hook runs lint-staged (ESLint fix on `.ts/.tsx/.astro`, Prettier on `.json/.css/.md`)
- Run `npx astro sync` before lint/build if types are stale

## Planning and task tracking

For all plans and multi-step work:

- Always create a task list with `[ ]` checkboxes for each task and subtask
- Mark tasks as `[x]` without unreasonable delay once complete — the list stays in sync with reality
- Use the task list to communicate progress: user can see what's done, what's in progress, and what's left
