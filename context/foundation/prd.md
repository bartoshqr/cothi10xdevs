...
> Decisions taken after the PRD was locked, during slice planning. Each entry **overrides** the referenced FR/US text; the original FR bodies are left intact for traceability. Read these as amendments to the FRs above.

### 2026-06-12 — S-05 planning (`multiround-edit-invalidation`)

1. **Re-evaluation ordering rule dropped (FR-026, US-04 AC line 87/94).** FR-026's "re-evaluating every still-existing invalidated mark is the mandatory first action … before they can add new Statements or submit their turn" is removed. During an active turn either party may edit, mark, re-mark, and add content **in any order**. The only enforcement is the existing submit gate: a turn cannot be submitted while any counterpart statement is unmarked or carries an invalidated mark. Re-evaluation is required *by turn-end*, not *ordered within* the turn. No insert-time guard is added (server enforces completeness at submit only).

2. **Orphaned counterpart statements keep their marks (FR-026, US-04 AC line 92).** Deleting your own statement no longer auto-clears the *counterpart's* prior mark on a preserved statement that referenced the deleted node. The preserved statement's content is unchanged, so the mark stands; only its **orphan status** (connectivity-to-root) is recomputed. This removes the relation-delete mark-clear cascade entirely. (The *deleted* statement's own marks still vanish via FK `ON DELETE CASCADE` — unchanged.) Orphaned statements are still preserved and explicitly highlighted as orphaned.

3. **Orphans appear in the divergence summary under an "orphaned" sub-label (FR-020, Business Logic).** Because orphan marks are now preserved (shift 2), an orphaned statement keeps its stance in the summary rather than being excluded. Within each summary section the entries are split into a standard group and an **orphaned** group (common ground → standard / orphaned; divergence → standard / orphaned; abstained → standard / orphaned) so a node disconnected from the root claim is visibly flagged without losing its stance.

4. **Final-round mini-turn always opens; no skip (FR-019).** FR-019's clause "if no invalidated marks and no unmarked advocate Statements exist after the advocate's final-turn submission, the mini turn is skipped and the exchange closes immediately" is **void**. The mini-turn always opens after the advocate's final-round submit (ratifies the as-built S-04 `submit_turn`, which never implemented the skip). The exchange closes on the challenger's mini-turn submit.

5. **Mini-turn permits change-of-mind on valid marks (FR-019, FR-026).** The mini-turn restriction "re-evaluate invalidated + mark unmarked only" is widened: during the mini-turn the challenger may **also revise marks on already-validly-marked statements**. Content writes (add/edit/delete) remain forbidden. Trade-off accepted: this makes the closing turn symmetric with a normal challenger turn, at the cost of the advocate not getting a response to a last-moment mark flip (the exchange closes on mini-turn submit).

6. **Advocate-initiated close (explicit + 7-day inactivity) deferred to new slice S-08 (FR-019).** S-05 ships **no advocate-initiated close**. Exchanges close in S-05 only via round-exhaustion / mini-turn (already shipped in S-04). FR-019's explicit-close path, the 7-day challenger-inactivity path, the activity-clock column, the countdown UI, and Abstain-defaulting of still-invalidated/unmarked statements at the moment of close all move to **S-08** (`advocate-close-and-timeout`). This supersedes Open Question 3's "Block: S-05" — it now blocks S-08.

7. **New — own-statement connectivity soft-guard (no prior FR; UI-only in S-05).** Before sending an invite (advocate) and before submitting a turn (either party), the UI warns and disables the action when the actor's **own** statements do not connect to the root claim, naming the dangling nodes. This reuses the orphan reachability computation (compute-at-read in the store). It is a **client-side guard only** in S-05 — not enforced server-side, and therefore bypassable via direct API by design; revisit if a hard server invariant is later wanted. The submit-time check scopes to the *submitter's own* statements; the counterpart's orphans remain a display concern (shifts 2–3), never a block on the other party.
